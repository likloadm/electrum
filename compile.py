...
PATH_SOURCES = PATH_ROOT / "electrum/sources"
...
def create_algorithm_ffi(algorithm, *, path, is_kem=False, is_sign=False, **kwargs):
    ...
    ffi.set_source(
        f"electrum.pqcrypto.{'_kem' if is_kem else '_sign'}.{algorithm}",
        f'#include "{ str(header_path.resolve()) }"',
        sources=['./electrum'+str(source.resolve()).split('electrum')[2] for source in (*common_sources, *variant_sources)],
